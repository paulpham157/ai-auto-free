...
class CursorStatus extends StatefulWidget {
  const CursorStatus({super.key});

  @override
  State<CursorStatus> createState() => _CursorStatusState();
}

class _CursorStatusState extends State<CursorStatus> {
  ...
  Future<void> _fetchCursorStatus() async {
    try {
      final response =
          await _dio.get('https://status.cursor.com/api/v2/status.json');
  ...
  Future<Response?> _fetchSummaryData() async {
    ...
    try {
      final response =
          await _dio.get('https://status.cursor.com/api/v2/summary.json');
      if (response.statusCode == 200) {
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Cursor ${S.of(context).status}: $_statusDescription',
